...
- **架构**: 基于领域驱动设计（DDD）原则的微服务架构
- **用途**: 提供高性能、可扩展的后端服务，支持 AI 智能体运行时、工作流引擎和业务逻辑

**前端 - React + TypeScript**
- **React 版本**: 18.2.0
- **TypeScript 版本**: 5.8.2
- **构建工具**: Rsbuild（基于 Rspack）用于快速开发和构建
- **UI 框架**: 基于 Arco Design 组件的自定义设计系统
- **用途**: 提供现代化、响应式的 Web 界面，支持可视化 AI 智能体开发

### 数据库技术

**主数据库 - MySQL**
- **版本**: 8.4.5
- **用途**: 存储应用程序数据、用户信息、智能体配置和业务实体的主要关系型数据库
- **特性**: 完整的 ACID 支持，支持复杂查询和事务

**缓存层 - Redis**
- **版本**: 8.0 (Bitnami)
- **用途**: 高性能缓存，用于会话管理、临时数据存储和应用性能优化

**搜索引擎 - Elasticsearch**
- **版本**: 支持 v7 和 v8
- **用途**: 为知识库提供全文搜索功能，支持文档索引和内容发现

**向量数据库 - Milvus**
- **用途**: 存储和管理向量嵌入，支持 AI/ML 功能，在知识库中实现语义搜索和相似度匹配

**对象存储 - MinIO**
- **用途**: S3 兼容的对象存储，用于文件、文档、图片和其他非结构化数据

### 核心框架与库

- **AI 运行时**: Eino 框架，用于 AI 智能体和工作流执行
- **工作流引擎**: FlowGram，用于可视化工作流构建
- **单仓库管理**: Rush.js，用于管理多个包
- **容器化**: Docker 和 Docker Compose，用于部署

该架构为开发者提供了高性能、高扩展性、易于二次开发的底层框架，助力开发者应对复杂的 AI 智能体开发需求。
